...
declare function local:permanent-ids($doc) {
    ...
    let $translation-id := translation:id($doc)
...
declare function local:log-event($type as xs:string, $event as xs:string, $object-type as xs:string, $uri as xs:string) {
    ...
    ,
        ...
        update insert <trigger event="{string-join(($type, $event, $object-type), "-")}" uri="{$uri}" timestamp="{current-dateTime()}" user="{ common:user-name() }"/> 
            into doc($log-uri)/log
    )
};
